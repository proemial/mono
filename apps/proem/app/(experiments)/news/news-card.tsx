...
function ppMouseMove(cardRef: React.RefObject<HTMLDivElement>, event: MouseEvent | React.MouseEvent<HTMLDivElement, MouseEvent>) {
	if (window.innerWidth <= 1024) return;

	const halfW: number = (cardRef.current?.clientWidth ?? 0) / 2;
	const halfH = (cardRef.current?.clientHeight ?? 0) / 2;
	const coorX = (halfW - ((event as MouseEvent).pageX - (cardRef.current?.offsetLeft ?? 0)));
	const coorY = (halfH - ((event as MouseEvent).pageY - (cardRef.current?.offsetTop ?? 0)));
...
function ppMouseLeave(cardRef: React.RefObject<HTMLDivElement>, event: MouseEvent | React.MouseEvent<HTMLDivElement, MouseEvent>) {
	if (window.innerWidth <= 1024) return;
...
export function NewsCard({
	...
	return (
		...
		>
			...
			>
				<div style={{ opacity: 0 }} className="ppNewsCard__notice absolute left-[0px] top-[-40px] w-full">
