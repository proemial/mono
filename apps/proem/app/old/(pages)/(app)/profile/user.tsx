...
export function getInternalUser(email?: string): { isInternal: boolean, proemialName?: string, proemialEmail?: string } {
    const mail = email ?? getInternalEmailFromCookie();

    const [name, domain] = mail?.split("@") ?? [];
    if (domain?.endsWith("proemial.ai") && name) {
        return { isInternal: true as const, proemialName: name as string, proemialEmail: mail };
    }
