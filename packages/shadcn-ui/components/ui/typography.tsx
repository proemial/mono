...
export function Paragraph({ children }: TypographyProps) {
	return <p className="text-base/relaxed break-words">{children}</p>;
}
...
export function Header2({ children }: TypographyProps) {
	return (
		<h2 className="scroll-m-20 text-2xl/normal font-normal">{children}</h2>
	);
}
...
export function Header5({ children }: TypographyProps) {
	return <h5 className="text-2xs uppercase">{children}</h5>;
}
