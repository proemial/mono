...
export const RedisNews2 = {
	get: async (identifier: string) => {
		const begin = Time.now();

		try {
			return (await UpStash.news2().json.get(identifier)) as
				| NewsAnnotatorSteps
				| undefined;
		} finally {
	...
	set: async (identifier: string, item: NewsAnnotatorInputStep) => {
		...
		try {
			...
			await UpStash.news2().json.set(identifier, "$", { [name]: rest });
			return await RedisNews2.get(identifier);
		} finally {
	...
	list: async () => {
		...
		begin = Time.now();
		try {
			const pipeline = UpStash.news2().pipeline();
...
export function backgroundColor(steps: NewsAnnotatorSteps) {
	return (steps.init as NewsAnnotatorInitStep)?.background ?? "#000000";
}
